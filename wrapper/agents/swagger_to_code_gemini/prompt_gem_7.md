...
**Objective**: Generate a complete, production-ready, cloud-native Spring Boot application based on a detailed technical specification (`*.md`). The generated code must be enterprise-grade, fully compliant with all specified requirements, and delivered as a single JSON object containing all project files.
...
> **Golden Rule:** The **Technical Specification (`.md` file) is the single source of truth.** If a requirement is missing, implement a sensible, modern enterprise default and document it in the `README.md`. **Do not leave any placeholders or "TODO" comments.**
...
-   **Dependency Source Code Rule:** For any feature where the technical specification provides a specific Maven dependency (e.g., a logging starter, , a resilience library), you **MUST NOT** generate the source code for that library's internal components. Your only task is to add the dependency to the `pom.xml` and perform the necessary configuration as instructed in the specification.
  ...
  - `MainApplication.java` – The main Spring Boot application entry point.Derive fileName according to input.
   (e.g., from the <artifactId> in the pom.xml, like MyProjectApplication.java)
...
### 3.1.1. `pom.xml` Generation Rules
-   **Source of Truth:** The `pom.xml` file **MUST** be constructed exclusively from the information provided in the technical specification (`.md` file). This includes:
    -   `<groupId>`, `<artifactId>`, `<version>`, `<name>`, and `<description>`.
    -   The exact parent POM, including its version (e.g., a specific `spring-boot-starter-parent`).
    -   The complete list of dependencies from the "Maven Dependencies" section.
    -   The complete list of plugins from the "Maven Plugins" section.
    -   All properties defined in the `<properties>` section.
-   **No Improvisation:** You **MUST NOT** add, remove, or change any dependency, plugin, or property that is not explicitly defined in the technical specification. Do not infer or add dependencies based on the code you are generating. The provided list is complete.
-   **Version Management:** You **MUST** use the exact versions specified for the parent POM, dependencies, and plugins. If a dependency version is managed by the parent POM or a BOM, you must not specify the version for that dependency. If the specification provides conflicting versions, you must prioritize the version defined in the parent POM or BOM.
...
-   **Strict Package Rule:** The Java source folder (`src/main/java`) **MUST** contain **only** the package structure (folders) defined in the technical specification's "Base Package Structure" section. You **MUST NOT** create any additional packages or sub-packages. The specified structure is exhaustive and complete.
...
-   **SOAP Integration**: If the technical specification requires integration with a SOAP service, you **MUST** implement the following:
    ...
    - Identify the controller, service which needs to implemnt SOAP integration and provide end to end implementation with actual logic.
...
### 3.14. Deployment
-   **Dockerfile:** Include **only** below content with suitable replacement of variables based on input
    ```
...
-   Is `pom.xml` in valid format?
-   **`pom.xml` Adherence Check:** Have you verified that the `pom.xml` was generated *only* using the information from the technical specification, with no added or inferred dependencies or plugins?
-   Is every requirement from the `.md` spec implemented?
