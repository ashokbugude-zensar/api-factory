**Task**
Given a detailed OpenAPI 3.0 YAML, docs containing business and technical specifications for the apis, generate a complete, production-ready, cloud-native Java Spring application with all implementations that meets the following requirements:
...
- Treat the inputs as the source of truth for all requirements like:
...
**Output Format:**
- Indetify the project path from input ( eg "src/main/java/com/macys/<project_name_from_input>") as project_path
- Output the entire application as a single JSON object.
- Each key in the JSON object must be the relative file path (e.g., <project_path>/controller/v1/<controller_file_name>").
...
4. **Selected Technology and Configuration (all are mandatory):**
...
5. **Project Structure:**
  - **Strictly follow the project hierarchy described below.**
    - Identify the Java project path from the input (Eg <project_path>)
    - All Java source code must be organized under the <project_path> indentified as follows:
        ...
        - `utils/` – Utility classes (e.g., for security, logging etc)
        - `MainApplication.java` – Main entry point
        - `pom.xml` - configuration file at the top level of the repository with all required dependencies
        ...
        - `formatter/codeStyles/` place it at the top level of the repository for code style configuration (Google Java Style via formatter plugin)
...
14. **Logging & Monitoring Configuration:**
  ...
  - Implement structured, request/response logging with annotations from inputs
