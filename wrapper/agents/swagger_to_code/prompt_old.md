...
- **Artifact (project) name:** `direct-to-store-carton-eapi` (kebab-case, aligned to domain & *-eapi naming)
- The service provides a modern REST/JSON wrapper over the **WEDIE010** legacy SOAP/XML service for **direct-to-store carton status** by **UCCID**, exposed through **Apigee**.
...
- Example:
  {
    ...
    "src/main/java/com/macys/ivm/controller/v1/DirectToStoreController.java": "package com.macys.ivm.controller.v1;\n// ...rest of the code...",
...
- All resource/configuration files must be under `src/main/resources/`:
  - `application.yml`
  - `logback-spring.xml`
  - `logback-access-spring.xml` (if applicable)
...
5. **Project Structure:**
  ...
  - Organize code under src/main/java/com/macys/iwm/ as follows:
...
8. **Key Features:**
  - Reactive REST API for all endpoints, headers, resource model, response code as listed in input swagger yaml, strictly follow implementation required for endpoints based on summary & description. Provide actual logic for all endpoints, services, and integrations.
   ...
   Purpose: Retrieve carton status by UCCID for direct-to-store operations via a REST/JSON wrapper over WEDIE010 SOAP.
   Endpoints (read-only):
   GET /api/v1/directToStore/carton/{UCCID}
   HEAD /api/v1/directToStore/carton/{UCCID} (existence/quick check)
   Required Request Headers:
   location, colleagueId, applicationId, division, store, device_type, device_id (all required)
   traceId (optional; if absent, generate and propagate)
   Response DTO:CartonDetailResponse
   Fields (types & semantics):
   returnCode: Integer — status code
   returnMessage: String — result message
   cartonNumber: String — carton identifier
   nafship: Long — NAF ship number
   bolstatus: String — BOL status
   bol: String — BOL number
   Sample Request Path:GET /api/v1/directToStore/carton/607809940059948981
  ...
  - Implement required services.
     - DirectToStoreService 
        - Purpose: Core business logic for retrieving carton details and interacting with the downstream SOAP service.
        - Definition: CartonDetailResponse invokeDirectToStore(String UCCID, Map<String, String> metaData, BigDecimal division,
        BigDecimal store, String mockResponse)
        - Functions:
...
16. **Endpoint Implementation:**
   - Implement `GET /v1/directToStore/carton/{cartonId}` with all required headers.
